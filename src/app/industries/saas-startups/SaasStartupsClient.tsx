...
import { ArrowRight, CheckCircle2, Send, Rocket, Code, Brain, BarChart3, Users, Zap, Eye, Layers, Target, TrendingUp, Shield, Repeat, Loader2 } from "lucide-react";
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      toast.success("Strategy Session Requested!", {
        description: "Our startup team will respond within 24 hours.",
      });
              ...
              {submitted ? (
                ...
                <form onSubmit={handleSubmit} className="p-8 rounded-2xl border border-border bg-card space-y-5">
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div><label className="text-xs font-semibold text-foreground/70 mb-1.5 block">Startup Name</label><input name="company" type="text" placeholder="Your Startup" className="w-full px-4 py-3 rounded-xl bg-background border border-border text-sm placeholder:text-muted-foreground/40 focus:outline-none focus:border-primary transition-colors" /></div>
                  </div>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div><label className="text-xs font-semibold text-foreground/70 mb-1.5 block">Email Address *</label><input required name="email" type="email" placeholder="john@startup.com" className="w-full px-4 py-3 rounded-xl bg-background border border-border text-sm placeholder:text-muted-foreground/40 focus:outline-none focus:border-primary transition-colors" /></div>
                  ...
                  <button type="submit" disabled={isSubmitting} className="w-full flex items-center justify-center gap-2 bg-primary text-primary-foreground px-8 py-4 rounded-full font-semibold text-sm hover:bg-primary/90 transition-all">
                    ...
                    {isSubmitting ? "Submitting..." : "Get Your Free Strategy Session"}
                  </button>
