      ...
      <div className="container mx-auto px-5 sm:px-6 relative z-10">
        <div className="max-w-3xl">
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
            <p className="text-sm font-bold tracking-[0.3em] uppercase text-primary mb-5">E-Commerce & Marketplace Enablement</p>
            <h1 className="text-3xl sm:text-4xl md:text-5xl lg:text-[3.5rem] font-bold text-hero-foreground leading-[1.08] mb-6">
              ...
              <span className="text-transparent bg-clip-text bg-gradient-to-r from-primary to-orange-300">Across Every Channel</span>
            </h1>
            <p className="text-hero-foreground/50 text-sm sm:text-base leading-relaxed mb-8 max-w-2xl">
              AI-powered commerce solutions that drive revenue across D2C, B2B, and global marketplaces like Amazon, Noon, and Trendyol, with predictive inventory, dynamic pricing intelligence, and conversion-optimized experiences that scale from launch to $100M+.
            </p>
            <div className="flex flex-col sm:flex-row gap-4 mb-10">
              <a href="#lead-capture" className="inline-flex items-center justify-center gap-2 bg-primary text-primary-foreground px-8 py-4 rounded-full font-semibold text-sm hover:bg-primary/90 transition-all hover:gap-3">
                Get Your Commerce Growth Plan <ArrowRight className="w-4 h-4" />
              </a>
            ...
            <div className="flex flex-wrap gap-x-6 gap-y-2">
              {["AI-Powered Pricing & Inventory", "Amazon · Noon · Trendyol · D2C", "Revenue Engineering, Not Just Design"].map((item) => (
                <span key={item} className="flex items-center gap-2 text-sm text-hero-foreground/40"><CheckCircle2 className="w-3.5 h-3.5 text-primary" />{item}</span>
              ))}
            </div>
          </motion.div>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 sm:gap-8">
          {stats.map((s, i) => (
            <motion.div key={s.label} initial={{ opacity: 0, y: 10 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }} className="text-center">
              ...
              <div className="text-sm text-section-dark-foreground/40 mt-1">{s.label}</div>
            </motion.div>
          ))}
        </div>
      </div>
